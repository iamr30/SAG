            ...
            <table class="table my-0">
              <thead>
                <tr class="text-center">
                  ...
                  <th @click="sort(2)">NOMBRE</th>
                  <th @click="sort(3)">ACTIVO</th>
                  <th>Acciones</th>
                </tr>
              </thead>
              <tbody>
...
export default {
  ...
  methods: {
    onClickEdit($idy) {
      const ediId = $idy;
      window.location.href = "/admin/usuarioscdu/editar/" + ediId + "/";
    ...
    searchButton() {
      ...
      if (this.search == "") {
        ...
        this.content.forEach((value, index) => {
          if (this.foundIt(value.username, this.search)) {
            this.aux.push(value);
    ...
    onClickDelete($idc, $iddel) {
      this.$confirm("Realmente desea eliminar el usuario", "Alerta", {
        ...
        .then(() => {
          axios.delete(`/admin/usuarioscdu/${$idc}`).then((response) => {
            if (response.data != 1) {
              this.showErrorNotification(
                "Error",
                "El usuario tiene pagos, condonaciones o tarjetas registradas"
              );
    ...
    addUser() {
      window.location.href = "/admin/usuarioscdu/agregar/";
    },
  },
};
</script>